    ...
    <?php include("header.php") ;
        $formValid = 1; 
        $nameErr = $calleErr = $apellidoErr = $dniErr = $departamentoErr = $emailErr = $passErr = $departamento = $nombre = $apellido = $calle = $pass = "";
        ...
        function registrar($email,$pass){
            $link = mysql_connect('localhost','root') or die('No se pudo conectar: '.mysql_error());
            mysql_select_db('bestnid',$link) or die('No se pudo seleccionar la base de datos');
            mysql_query("INSERT INTO usuario(email,password) VALUES ('$email','$pass')") or die("Falló al intentar registrar el usuario");
            //mysql_query("INSERT INTO registrado(email,nombre,apellido,dni,fechaNacimiento,borrado,sexo,calle,numCalle) VALUES()");
            mysql_close($link);
        }
